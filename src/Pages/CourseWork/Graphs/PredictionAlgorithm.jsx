...
let calculateStockValue = (z, i) => {
    ...
    if (sd >= tssd[tssd.length * 0.25] && sd <= tssd[tssd.length * 0.75]) {
        // nice multipler
        mult = 1.25;
    }

    // checks if the pearson coefficient is within the 75th percentile
    if (pearson >= tpa[tpa.length * 0.75]) {
        mult += 0.5;
    }

    mult += 0.1 * numOfOutliers;
...
let grabPearsonsCoefficient = (stock, z, mu) => {
    // return stock's pearson coefficient
